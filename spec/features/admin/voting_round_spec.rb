...
describe "admin's voting round actions" do
  ...
  describe "create new voting round" do
    ...
    context "when label field is not empty" do
      before do
    ...
    context "when label field is empty" do
      ...
      it "displays default label text on index page as the label" do
        @show_admin_voting_round_page = Admin::ShowVotingRound.new
        @show_admin_voting_round_page.load(id: VotingRound.last.id)
        @show_admin_voting_round_page.label.text.should == @default_label_text
      end

      it "displays default label text on voting round show page" do
        @admin_voting_rounds_page = Admin::VotingRounds.new
        @admin_voting_rounds_page.load
  ...
  describe "edit voting round" do
    context "when label is changed" do
      ...
      it "displays updated label text on voting round show page" do
        @show_admin_voting_round_page = Admin::ShowVotingRound.new
        @show_admin_voting_round_page.load(id: @voting_round.id)
        @show_admin_voting_round_page.label.text.should == "new label text"
      end
    end
  end
end
